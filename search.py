...
def tree_search(problem, fringe):
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            ...
            print "Coste del recorrido: ", node.path_cost
            return node
...
def branch_bound_tree_search(problem):
    return tree_search(problem, SortedList())

def branch_bound_heuris_tree_search(problem):
    return tree_search(problem, SortedListH(problem))
...
"""
    def hBad(self, node):
        ...
        if locs:
            res=0
            for a,b in zip(locs[node.state],locs[self.goal]):
                res+=abs(a-b)
            return res
        else:
            return infinity
